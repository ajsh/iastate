...
class Theme
{
	...
	protected function configure()
	{
		$this->setOptions(array(
			...
			'request_uri'            => null,
			'navbar'                 => null,
			'navbar_classes'         => 'navbar-large navbar-no-collapse',
			'sidebar'                => null,
			...
			'head_link' => array(
				...
				'font-awesome' => '//netdna.bootstrapcdn.com/font-awesome/4.6.1/css/font-awesome.css',
			),
			'head_style' => array(),
			'head_script' => array(
				'file' => array(),
...
<ul class="wd-l-TopStripMenu-group wd-l-TopStripMenu-group--signons first">
	<li>
		<span class="wd-l-TopStripMenu-header">Sign-ons <span class="fa fa-angle-down"></span></span>
...
<ul class="wd-l-TopStripMenu-group wd-l-TopStripMenu-group--directory last">
	<li>
		<span class="wd-l-TopStripMenu-header">More <span class="fa fa-angle-down"></span></span>
		<ul class="wd-l-TopStripMenu-toolbar">
...
HTML;
	...
	public function renderNavigationBootstrap($pages, $depth = 0, $activePages = array(), $tabs = false)
	{
		if (is_scalar($pages))
		{
			$navbar = $this->getOption('navbar');
			$pages = explode('/', $pages);
			while ($pages)
			{
		...
		foreach ($index as $i)
		{
			...
			$attr['class'] = implode(' ', $attr['class']);
			$html[] = '<' . $tag . ' ' . $this->createAttributesString($attr) . '>';
			$html[] = $icon ? "<span class=\"fa fa-$icon\" title=\"$label\"></span>" : '';
			$html[] = '<span' . ($icon ? ' class="icon-label"' : '') . ">$label</span></$tag>";
	...
	/**
	 * Render next level light gray nav bar
	 *
	 * @param       $pages
	 * @param int   $depth
	 ...
	 * @return string
	 */
	public function renderSubnavBootstrap($pages, $depth = 0, $activePages = [], $tabs = FALSE)
	{
		$index = $this->sort($pages);
		$activePages = [];
		$subnav = [];
		...
		foreach ($pages as $i => $page)
		{
			if ($i == $activePages[$i]['value'] > 0)
			{
				if($page['pages'])
				{
					$activeSubpages = $activePages[$i]['pages'];
					foreach ($page['pages'] as $name => $subPage)
					{
						$showchildren = $subPage['showchildren'] ? $subPage['showchildren'] : ($subPage['show_children'] ? $subPage['show_children'] : FALSE);
						$active = $activeSubpages[$name]['value'];
						if ($showchildren != FALSE || $active == 100)
						{
							foreach ($subPage['pages'] as $tab => $tabPage)
							{
								$activeTabs = $activeSubpages[$name]['pages'];
								$subNav[] = $this->renderNavigationBootstrap($subPage['pages'], 1, $activeTabs, TRUE);
								break;
							}
						}
					}
